...
export function boardUsed(jc) {
  ...
  // Every consumption row whose material is NOT the board this job is planned
  // on, folded to one line per material with the total actually issued.
  const byMaterial = new Map();
  for (const i of jc.issues || []) {
    const mid = idOf(i.material_id);
    if (mid == null || usedId == null || mid === usedId) continue;
