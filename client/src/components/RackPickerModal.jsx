...
import { defaultPickSelection, duplicatePickAssets, pickPayload } from '../lib/plateRack.js';
...
export default function RackPickerModal({ open, requestNumber, lines = [], busy = false, onCancel, onConfirm }) {
            ...
            <section key={line.component_id} className="ci-form-panel">
              ...
              <div>
                {candidates.map(row => {
                  ...
                  return (
                    <label key={row.id}
                      className={`flex cursor-pointer items-center gap-3 border-b border-slate-100 py-2 last:border-0 ${clash ? 'bg-red-50/60' : ''}`}>
