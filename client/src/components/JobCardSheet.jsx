// The printable job-card traveler itself — ONE A4 sheet that walks the floor
// with the job: spec, materials issued, and the sign-off strip.
//
// This lives in its own component because the traveler is now printed from TWO
// places: /production/jobcard/:id (one card) and /production/jobcards/print
// (every card a planner ticked, stacked into one print run). Both render THIS,
// so a batch page can never drift from the single page — there is one layout,
// not a printed copy of a printed copy. Anything a traveler must say is said
// here; the pages around it own only the toolbar and the page breaks.
//
...
import { shadeAge } from './ui.jsx';
import { scLabel } from '../pages/shade-cards/lifecycle.js';
...
export default function JobCardSheet({ jc }) {
  const t = (fam) => (jc.tools || []).filter(x => x.family === fam);
  const block = t('block')[0];
    ...
    <div className="jc-sheet rounded-2xl border border-slate-200 bg-white p-8 shadow-card print:border-0 print:shadow-none">
      ...
      <div className="jc-head flex items-start justify-between border-b-2 border-ink-900 pb-4">
        <div>
          ...
          {/* The number and the product name are read together on the floor, at
              the same 20px — the number came down a step from text-2xl, the name
              up from ProductIdentity's compact text-xs. The WEIGHT is what
              separates them now, and it points at the product: the name is what
              an operator is looking for, the number is how it is filed.

              `!font-bold` is important for the same reason the size once was, in
              a property `declaresFontSize` does not cover: Tailwind emits the
              font-weight utilities ALPHABETICALLY, so `.font-semibold` — which
              ProductIdentity always sets — lands after every other weight and
              beats all of them. There is no plain class that can win here. The
