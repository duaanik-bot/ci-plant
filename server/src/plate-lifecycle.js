...
export async function assertPlateReadyForPrinting(qc, jobCardId) {
  const rows = await qc(`SELECT prc.status
    FROM tooling_requests tr JOIN plate_request_components prc ON prc.tooling_request_id=tr.id
    WHERE tr.job_card_id=$1 AND tr.family='plate' AND prc.status<>'cancelled'`, [jobCardId]);
  if (!rows.length) return { required: 0, ready: 0, is_ready: true };
  const summary = plateReadinessSummary(rows);
  if (!summary.is_ready) {
    throw Object.assign(new Error(`Plates are not ready — ${summary.ready} of ${summary.required} available`), {
      status: 409,
      body: { code: 'PLATES_NOT_READY', plates: summary },
    });
  }
  return summary;
}
