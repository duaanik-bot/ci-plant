// Dashboard KPIs + Reports. Live queries — no refresh buttons, no pivots.
import { Router } from 'express';
import db from '../db.js';

const r = Router();

r.get('/dashboard', (req, res) => {
  const month = new Date().toISOString().slice(0, 7);

  const ordersInHand = db.prepare(`
    SELECT COUNT(*) AS lines, COALESCE(SUM((qty-dispatched_qty)*rate),0) AS value,
           COALESCE(SUM(qty-dispatched_qty),0) AS qty
    FROM order_lines WHERE status NOT IN ('dispatched','cancelled')`).get();

  const wip = db.prepare(`
    SELECT COUNT(*) AS jobs FROM job_cards WHERE status IN ('open','in_progress')`).get();

  const wipByStage = db.prepare(`
    SELECT js.stage, COUNT(*) AS n FROM job_stages js
    JOIN job_cards jc ON jc.id=js.job_card_id
    WHERE jc.status IN ('open','in_progress') AND js.status='in_progress'
    GROUP BY js.stage`).all();

  const producedMonth = db.prepare(`
    SELECT COALESCE(SUM(qty_produced),0) AS qty FROM job_cards
    WHERE status='closed' AND substr(closed_at,1,7)=?`).get(month);

  const scrap = db.prepare(`
    SELECT COALESCE(SUM(js.qty_scrap),0) AS scrap, COALESCE(SUM(js.qty_in),0) AS input
    FROM job_stages js WHERE js.status='completed' AND substr(js.completed_at,1,7)=?`).get(month);

  const shortages = db.prepare(`
    SELECT COUNT(*) AS n FROM (
      SELECT m.id FROM materials m
      LEFT JOIN (SELECT material_id, SUM(qty) q FROM stock_batches WHERE status='available' GROUP BY material_id) av ON av.material_id=m.id
      LEFT JOIN (SELECT p.board_material_id mid, SUM(ol.sheets_required) q FROM order_lines ol
                 JOIN products p ON p.id=ol.product_id WHERE ol.status IN ('planned','ready') GROUP BY p.board_material_id) dm ON dm.mid=m.id
      WHERE COALESCE(av.q,0) < COALESCE(dm.q,0) OR COALESCE(av.q,0) < m.reorder_level)`).get();

  const readyDispatch = db.prepare(`
    SELECT COUNT(*) AS lines, COALESCE(SUM((ol.qty-ol.dispatched_qty)*ol.rate),0) AS value
    FROM order_lines ol WHERE ol.status='produced'`).get();

  const onTime = db.prepare(`
    SELECT COUNT(*) AS total,
      SUM(CASE WHEN date(d.dispatched_at) <= date(o.delivery_date) THEN 1 ELSE 0 END) AS ontime
    FROM dispatches d JOIN orders o ON o.id=d.order_id
    WHERE o.delivery_date IS NOT NULL AND substr(d.dispatched_at,1,7)=?`).get(month);

  const dispatchedMonth = db.prepare(`
    SELECT COALESCE(SUM(dl.qty*ol.rate),0) AS value FROM dispatch_lines dl
    JOIN dispatches d ON d.id=dl.dispatch_id
    JOIN order_lines ol ON ol.id=dl.order_line_id
    WHERE substr(d.dispatched_at,1,7)=?`).get(month);

  const machines = db.prepare(`
    SELECT m.id, m.name, m.type, m.status,
      (SELECT COUNT(*) FROM job_cards jc WHERE jc.machine_id=m.id AND jc.status IN ('open','in_progress')) AS active_jobs
    FROM machines m ORDER BY m.type, m.name`).all();

  const alerts = [];
  const shortLines = db.prepare(`
    SELECT ol.id, p.name AS product_name, o.po_number, ol.sheets_required, m.name AS board_name,
      COALESCE(av.q,0) AS available
    FROM order_lines ol
    JOIN products p ON p.id=ol.product_id
    JOIN materials m ON m.id=p.board_material_id
    JOIN orders o ON o.id=ol.order_id
    LEFT JOIN (SELECT material_id, SUM(qty) q FROM stock_batches WHERE status='available' GROUP BY material_id) av ON av.material_id=p.board_material_id
    WHERE ol.status IN ('planned','ready') AND COALESCE(av.q,0) < ol.sheets_required
    LIMIT 5`).all();
  for (const s of shortLines) {
    alerts.push({ type: 'shortage', text: `${s.board_name} short for ${s.product_name} (PO ${s.po_number}) — need ${s.sheets_required}, have ${Math.round(s.available)}` });
  }
  const dueSoon = db.prepare(`
    SELECT o.po_number, c.name AS customer_name, o.delivery_date
    FROM orders o JOIN customers c ON c.id=o.customer_id
    WHERE o.status='open' AND o.delivery_date <= date('now','+3 day')
    ORDER BY o.delivery_date LIMIT 5`).all();
  for (const d of dueSoon) {
    alerts.push({ type: 'due', text: `PO ${d.po_number} (${d.customer_name}) due ${d.delivery_date}` });
  }
  const awPending = db.prepare(`
    SELECT COUNT(*) AS n FROM order_lines WHERE status='planned' AND artwork_locked=0`).get();
  if (awPending.n > 0) alerts.push({ type: 'artwork', text: `${awPending.n} line(s) waiting for artwork approval` });

  const recentJobs = db.prepare(`
    SELECT jc.jc_number, jc.status, p.name AS product_name, c.name AS customer_name, jc.qty_planned,
      (SELECT stage FROM job_stages WHERE job_card_id=jc.id AND status='in_progress' LIMIT 1) AS current_stage,
      (SELECT COUNT(*) FROM job_stages WHERE job_card_id=jc.id AND status='completed') AS done_stages,
      (SELECT COUNT(*) FROM job_stages WHERE job_card_id=jc.id) AS total_stages
    FROM job_cards jc JOIN products p ON p.id=jc.product_id
    JOIN order_lines ol ON ol.id=jc.order_line_id
    JOIN orders o ON o.id=ol.order_id JOIN customers c ON c.id=o.customer_id
    WHERE jc.status IN ('open','in_progress') ORDER BY jc.id DESC LIMIT 8`).all();

  res.json({
    orders_in_hand: ordersInHand,
    wip_jobs: wip.jobs,
    wip_by_stage: wipByStage,
    produced_month: producedMonth.qty,
    scrap_pct: scrap.input > 0 ? +(100 * scrap.scrap / scrap.input).toFixed(1) : 0,
    shortages: shortages.n,
    ready_dispatch: readyDispatch,
    dispatched_month_value: dispatchedMonth.value,
    on_time_pct: onTime.total > 0 ? Math.round(100 * onTime.ontime / onTime.total) : null,
    machines, alerts, recent_jobs: recentJobs,
  });
});

// ── Reports ─────────────────────────────────────────────────────────────────
r.get('/reports/production', (req, res) => {
  const { from, to } = req.query;
  res.json(db.prepare(`
    SELECT jc.jc_number, jc.closed_at, p.name AS product_name, c.name AS customer_name,
           jc.qty_planned, jc.qty_produced, jc.qty_scrap, jc.sheets_issued, m.name AS machine_name,
           ROUND(100.0*jc.qty_produced/(jc.qty_planned),1) AS fulfilment_pct
    FROM job_cards jc
    JOIN products p ON p.id=jc.product_id
    JOIN order_lines ol ON ol.id=jc.order_line_id
    JOIN orders o ON o.id=ol.order_id JOIN customers c ON c.id=o.customer_id
    LEFT JOIN machines m ON m.id=jc.machine_id
    WHERE jc.status='closed' AND date(jc.closed_at) BETWEEN COALESCE(?, date('now','-30 day')) AND COALESCE(?, date('now'))
    ORDER BY jc.closed_at DESC`).all(from || null, to || null));
});

r.get('/reports/scrap', (req, res) => {
  res.json(db.prepare(`
    SELECT js.stage, COUNT(*) AS runs, SUM(js.qty_in) AS input, SUM(js.qty_scrap) AS scrap,
           ROUND(100.0*SUM(js.qty_scrap)/NULLIF(SUM(js.qty_in),0),2) AS scrap_pct
    FROM job_stages js WHERE js.status='completed'
    GROUP BY js.stage ORDER BY scrap_pct DESC`).all());
});

r.get('/reports/sales', (req, res) => {
  res.json(db.prepare(`
    SELECT c.name AS customer_name, c.segment,
           COUNT(DISTINCT o.id) AS orders,
           SUM(ol.qty*ol.rate) AS order_value,
           SUM(ol.dispatched_qty*ol.rate) AS dispatched_value,
           SUM((ol.qty-ol.dispatched_qty)*ol.rate) AS pending_value
    FROM order_lines ol
    JOIN orders o ON o.id=ol.order_id JOIN customers c ON c.id=o.customer_id
    WHERE ol.status != 'cancelled'
    GROUP BY c.id ORDER BY order_value DESC`).all());
});

r.get('/reports/dispatch-register', (req, res) => {
  res.json(db.prepare(`
    SELECT d.challan_number, d.dispatched_at, d.vehicle, c.name AS customer_name, o.po_number,
           SUM(dl.qty) AS total_qty, SUM(dl.qty*ol.rate) AS value
    FROM dispatches d
    JOIN customers c ON c.id=d.customer_id
    JOIN orders o ON o.id=d.order_id
    JOIN dispatch_lines dl ON dl.dispatch_id=d.id
    JOIN order_lines ol ON ol.id=dl.order_line_id
    GROUP BY d.id ORDER BY d.id DESC`).all());
});

r.get('/reports/machine-load', (req, res) => {
  res.json(db.prepare(`
    SELECT m.name, m.type, m.capacity_per_hour, m.status,
      COUNT(jc.id) AS jobs_30d, COALESCE(SUM(jc.qty_produced),0) AS produced_30d,
      COALESCE(SUM(jc.qty_scrap),0) AS scrap_30d
    FROM machines m
    LEFT JOIN job_cards jc ON jc.machine_id=m.id AND jc.closed_at >= datetime('now','-30 day')
    GROUP BY m.id ORDER BY m.type, m.name`).all());
});

r.get('/audit', (req, res) => {
  res.json(db.prepare('SELECT * FROM audit_log ORDER BY id DESC LIMIT 200').all());
});

export default r;
