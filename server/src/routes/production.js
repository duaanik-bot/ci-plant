...
const JC_VIEW = `
  SELECT jc.*, p.name AS product_name, p.code AS product_code, p.party_item_code, p.size,
         ...
         -- The line a card reads its spec from: its own, or the gang's lead
         -- member for a parent card that has none. Same rule the print board
         -- uses, and what the board verdict is keyed on.
...
r.post('/job-stages/:id/start', canRun, async (req, res, next) => {
  try {
    await tx(async (qc, oc) => {
      ...
      if (st.stage === 'printing') {
        // Plates INFORM a start, they never refuse one. The warehouse says what
        // it knows and the record says what was short; the man at the press
        // decides. Whatever IS matched still gets issued — a start that goes
        // ahead with plates missing does not lose the plates it did have.
        const plates = await plateReadinessForPrinting(qc, jc.id);
        if (!plates.is_ready && plates.required) {
          await audit('job_stage', st.id, 'plates_short_at_start',
            ...
            }${plates.request_numbers.length ? ` (${plates.request_numbers.join(', ')})` : ''}`,
            qc, req.user.name);
        }
...
r.get('/print-planning', async (_req, res, next) => {
  try {
    ...
    // Gates are computed once per card and feed BOTH the board verdict and the
    // traffic light, so the two can never describe different facts.
    const gatesByCard = new Map();
    for (const cRow of cards) {
      const line = anchorById.get(cRow.anchor_line_id);
      if (line) gatesByCard.set(cRow.id, await readiness(line, one, rctx));  // served from rctx
    }
    await stampBoardState(cards, {
      lineIdOf: c => c.anchor_line_id,
      gangIdOf: c => c.gang_run_id,
      gatesOf: c => gatesByCard.get(c.id),
    });
    await stampPlateState(cards, {
      jobCardIdOf: c => c.id,
      gangIdOf: c => c.gang_run_id,
    });
    for (const cRow of cards) {
    ...
    // (stampBoardState already gave every member of a gang the run's weakest
