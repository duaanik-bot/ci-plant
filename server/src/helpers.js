...
import { isExpiredByAge } from './shade-flow.js';
...
export async function shadeCardsFor(productIds, qf = q) {
  ...
  // product wins; status rides along so Planning and Invoicing can show the
  // live approval state, not just the age. revision_no is dropped from the
  // select — nothing downstream consumes it any more. The old status NOT IN
  // ('superseded','archived') filter is gone too: those statuses no longer
  // exist (the migration remapped every card to draft/sent/approved/rejected),
  // and any card that held one of them was set active=0, so active=1 alone
  ...
  // Master itself. products.shade_card_number/date is now a DERIVED CACHE of
  // the module rather than a rival source, so this fallback only fires for a
  // product whose managed card was retired — returning the retired number here
  // is the honest answer, not a stale duplicate. A managed card (above) wins,
  // so the two sources never fight — master rows are folded in first and a
  // shade_cards row for the same product overwrites it.
...
export async function readinessBatch(lines, oc = one, qc = q) {
  ...
  const [materials, batches, tools, shades, incoming, fg] = await Promise.all([
    ...
    // revision_no dropped and the status filter gone — same reasoning as
    // shadeCardsFor above: nothing reads revision_no any more, and
    // 'superseded'/'archived' no longer exist as statuses (migrated cards were
    // set active=0), so active=1 alone covers the same ground.
    qc(`SELECT DISTINCT ON (product_id) product_id, sc_number, status, creation_date
        FROM shade_cards
        WHERE product_id = ANY($1) AND active=1
...
export async function readiness(line, oc = one, ctx = null) {
  ...
  // chip keeps showing it: registered but not approved (or approved-and-expired)
  // blocks softly; untracked informs; approved and in-date reads ready enough
  // for the job-card gate. hard stays false here on purpose and unchanged: this
  // gate is what makes PLANNING nag, not what stops a press — the readiness
  // LIGHT is where shade is hard (see readiness-light.js), and the real,
  // unconditional production stop is printingEligibility() at printing start.
  //
  // revision_no dropped from the query — nothing reads it any more (condition
  // used to print "Rev N" and now has nothing to say). The old status NOT IN
  // ('superseded','archived') filter is gone too: those statuses no longer
  // exist, and the migration set any card that held one of them active=0, so
  // active=1 alone covers the same ground.
  const shade = ctx
