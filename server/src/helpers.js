...
export async function readiness(line, oc = one, ctx = null) {
  ...
  // The Plate Set shows on the Tooling chip and cannot hold a line back. It is
  // `hard: false`, which now means exactly what it reads as: toolingGateOk()
  // ignores a soft family's status entirely. That was NOT true when the plate
  // set was first folded in — the gate failed a soft family on 'not_ready' too,
  // so an unready plate set quietly stopped orders.js promoting a line planned →
  // ready with nothing on screen explaining it, and the whole module had to be
  // detached (9aedf20). Softening the gate is what let it come back.
