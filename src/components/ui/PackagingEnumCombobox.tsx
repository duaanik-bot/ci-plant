...
export function PackagingEnumCombobox({
  ...
  return (
    <div ref={containerRef} className={`relative min-w-0 ${className}`}>
      <div
        className={`flex items-center gap-0.5 rounded bg-ds-elevated/40 shadow-sm transition-colors ${
          disabled
      ...
      >
        <input
          ...
          className={`min-w-0 flex-1 border-0 bg-transparent py-1.5 pl-2 pr-1 text-sm text-ds-ink outline-none placeholder:text-ds-ink-faint ${inputClassName}`}
        ...
        <button
          ...
          tabIndex={-1}
          disabled={disabled}
          className="shrink-0 rounded-r px-1.5 py-1.5 text-ds-ink-faint hover:bg-neutral-100"
      ...
      {open && !disabled ? (
        <ul
          id={listId}
          role="listbox"
          className="absolute left-0 right-0 top-full z-50 mt-0.5 max-h-48 overflow-auto rounded-ds-sm bg-ds-elevated py-0.5 shadow-lg"
        >
          {allowEmpty ? (
            <li
              role="option"
              className={`cursor-pointer px-2 py-1.5 text-xs text-ds-ink-faint hover:bg-ds-main/60 ${optionClassName}`}
              onMouseDown={(e) => e.preventDefault()}
              onClick={() => onPick('')}
            >
              {emptyLabel}
          ...
          {filtered.map((opt) => (
            <li
              key={opt}
              role="option"
              className={`cursor-pointer px-2 py-1.5 text-xs text-ds-ink hover:bg-ds-main/60 ${
          ...
          {filtered.length === 0 ? (
            <li className={`px-2 py-2 text-xs text-ds-ink-faint ${optionClassName}`}>
