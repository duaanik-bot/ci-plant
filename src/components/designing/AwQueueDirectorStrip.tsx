    ...
    <div className="border-b border-slate-800 bg-background px-3 py-2">
      <div className="max-w-7xl mx-auto w-full flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between lg:gap-4">
        <div className="flex flex-wrap items-center gap-2">
          <label className="inline-flex items-center gap-2 rounded-md border border-slate-800 bg-background px-2.5 py-1.5 text-[11px] font-semibold text-slate-200 cursor-pointer select-none">
            <input
              type="checkbox"
          ...
          {showRecallJob ? (
            <Dialog.Root open={recallOpen} onOpenChange={setRecallOpen}>
              <Dialog.Trigger asChild>
                <button
                  type="button"
                  className="rounded-md bg-amber-600 hover:bg-amber-500 px-2.5 py-1.5 text-[11px] font-semibold text-primary-foreground disabled:opacity-50"
                  disabled={recallBusy}
                >
                ...
                >
                  ...
                  <div className="mt-4 flex justify-end gap-2">
                    ...
                    <button
                      type="button"
                      disabled={recallBusy}
                      className="rounded-md bg-amber-600 hover:bg-amber-500 px-3 py-1.5 text-xs font-semibold text-primary-foreground disabled:opacity-50"
                      onClick={() => {
                        void (async () => {
