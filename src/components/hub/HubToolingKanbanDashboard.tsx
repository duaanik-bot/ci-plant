...
function BoardZoneTitle({
  ...
  return (
    <div className="flex flex-col gap-1 min-w-0">
      <h2 className="text-sm font-semibold uppercase tracking-wide text-ds-warning flex flex-wrap items-baseline gap-x-1.5">
        <span>{name}</span>
        <span
          className={`tabular-nums font-bold ${
            count === 0 ? 'text-[var(--text-secondary)]' : 'text-ds-warning'
          }`}
        >
          ({count})
        </span>
      </h2>
      <p className="text-xs uppercase tracking-wider text-[var(--text-secondary)] font-semibold tabular-nums leading-tight shrink-0">
        {unitCount} units
      </p>
    </div>
  )
...
function DieHubZoneSummaryBar({
  ...
  const Card = ({
    ...
    <div
      className={`flex-1 min-w-[132px] rounded-lg border ${borderClass} bg-ds-main/95 px-3 py-2.5 flex items-center gap-2.5`}
    >
      <span className={`h-2.5 w-2.5 rounded-full shrink-0 ${dotClass}`} aria-hidden />
      <div className="min-w-0">
        <p className="text-xs uppercase tracking-wide text-[var(--text-secondary)] font-semibold leading-tight">
          {label}
        </p>
        <p
          className={`text-lg font-bold tabular-nums leading-tight ${
            count === 0 ? 'text-[var(--text-secondary)]' : 'text-foreground'
          }`}
        >
          ({count})
        </p>
  ...
  return (
    <div
      className="relative z-10 mb-1 flex flex-wrap gap-2 rounded-xl border border-ds-line/50 bg-ds-main/95 backdrop-blur-sm px-3 py-3 shadow-[0_4px_24px_rgba(0,0,0,0.35)]"
      role="region"
    ...
    >
      <Card
        ...
        borderClass="border-ds-warning/50"
      />
      <Card
        label="Outside vendor"
        count={outsideVendor}
        dotClass="bg-[var(--brand-primary)] shadow-ds-depth-sm"
        borderClass="border-[var(--brand-primary)]/55"
      />
      <Card
        label="Live inventory"
        count={liveInventory}
        dotClass="bg-[var(--success)] shadow-ds-depth-sm"
        borderClass="border-[var(--success)]/55"
      />
      <Card
  ...
  return (
    <p
      className={`mt-1.5 text-xs leading-tight ${
        stale ? 'text-[var(--error)] font-medium' : 'text-[var(--text-secondary)]'
      }`}
    >
      Time in queue: {age}
    </p>
...
function HubTriageTime({ at }: { at: string | null | undefined }) {
  return (
    <p className="mt-1.5 text-xs leading-tight text-[var(--text-secondary)]">
...
export default function HubToolingKanbanDashboard({ mode }: { mode: 'dies' | 'blocks' }) {
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      ...
      if (el) {
        ...
        el.classList.add('ring-2', 'ring-ds-warning/35', 'ring-offset-2', 'ring-offset-black')
        window.setTimeout(() => {
          el.classList.remove('ring-2', 'ring-ds-warning/35', 'ring-offset-2', 'ring-offset-black')
        }, 2400)
        return
      }
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      ...
      if (el) {
        el.scrollIntoView({ block: 'center', behavior: 'smooth' })
        el.classList.add('ring-2', 'ring-ds-success/60', 'ring-offset-2', 'ring-offset-black')
        window.setTimeout(() => {
          el.classList.remove('ring-2', 'ring-ds-success/60', 'ring-offset-2', 'ring-offset-black')
        }, 2400)
        return
      }
  ...
  function renderSpecs(r: ToolRow, zone: 'triage' | 'prep' | 'inventory' | 'custody') {
    if (r.kind === 'die') {
      return (
        <>
          <div className="mt-1 space-y-0.5 text-xs font-medium text-[var(--text-secondary)] leading-tight">
            ...
            <p>
              Die type (master):{' '}
              <span className="text-[var(--text-secondary)]">{r.masterType?.trim() || '—'}</span>
            </p>
            <p className="text-xs text-[var(--text-secondary)]">
              Material: <span className="text-[var(--text-secondary)]">{r.materialLabel}</span>
            </p>
            {r.location ? (
              <p className="text-xs text-[var(--text-secondary)]">Rack: {r.location}</p>
            ) : null}
          </div>
          <details className="mt-1.5 rounded border border-ds-line/40 bg-ds-main/50 px-2 py-1">
            <summary className="text-xs font-semibold uppercase tracking-wide text-[var(--text-secondary)] cursor-pointer select-none">
              Technical data
            </summary>
    ...
    return (
      <div className="mt-1 space-y-0.5 text-xs font-medium text-[var(--text-secondary)]">
        {zone === 'triage' && r.triageAwReference ? (
          <p>
            AW: <span className="text-ds-ink">{r.triageAwReference}</span>
          </p>
        ) : null}
        {zone === 'triage' && r.triageBlockDimensions ? (
          <p>
            Block / sheet: <span className="text-ds-ink">{r.triageBlockDimensions}</span>
          </p>
        ) : null}
        <p>
          Type: <span className="text-[var(--text-secondary)]">{r.typeLabel}</span>
        </p>
        <p>
          Material: <span className="text-[var(--text-secondary)]">{r.materialLabel}</span>
          <span className="text-[var(--text-secondary)] ml-1">(Brass · Magnesium · Copper)</span>
        </p>
        {r.reliefDepthMm != null && Number.isFinite(r.reliefDepthMm) ? (
          <p className={HUB_EMB_MONO}>
            Relief depth: <span className="text-ds-ink">{r.reliefDepthMm} mm</span>
          </p>
        ) : null}
        <p
          className={`${HUB_EMB_MONO} ${r.strikeOverLimit ? 'text-[var(--error)] animate-pulse font-semibold' : 'text-[var(--brand-primary)]'}`}
        >
          Strikes: {r.cumulativeStrikes.toLocaleString()} / {r.strikeLimit.toLocaleString()}
        </p>
        {r.blockSize ? (
          <p>
            Size: <span className="text-[var(--text-secondary)]">{r.blockSize}</span>
          </p>
        ) : null}
        {r.storageLocation ? (
          <p className="text-xs text-[var(--text-secondary)]">Rack: {r.storageLocation}</p>
        ) : null}
      </div>
    )
  }
  ...
  function renderCard(
    ...
    const strikeAlert =
      r.kind === 'emboss' && r.strikeOverLimit
        ? ' ring-1 ring-[var(--error)]/70 border-[var(--error)]/80 animate-pulse'
        : ''
    const liClass = `rounded-lg border bg-card p-2 overflow-visible ${
      custodyInUse
        ? 'border-2 border-[var(--brand-primary)] hub-tool-in-use-pulse'
        : zone === 'custody' && r.jobCardHub?.key === 'printed'
          ? 'border-[var(--success)]/40 shadow-ds-depth-sm'
          : 'border-ds-line/40'
    ...
    if (r.kind === 'die') {
      ...
      return (
        ...
        >
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              {hasTypeMismatch ? (
                <button
                  ...
                  className="shrink-0 rounded border border-[var(--error)]/50 bg-[var(--error-bg)] px-1.5 py-0.5 text-xs font-bold uppercase tracking-wider text-[var(--error)] hover:opacity-90"
                >
                  Type mismatch
                </button>
              ) : hasSimilar ? (
          ...
          <p className="text-xs text-[var(--text-secondary)] truncate mt-1" title={r.title}>
            {r.title}
          </p>
          {r.hubConditionPoor ? (
          ...
          {zone === 'custody' ? (
            <div className="flex flex-wrap items-center gap-x-2 gap-y-1 mt-0.5">
              ...
              {r.hubCustodySource === 'rack' ? (
                <span className="inline-flex items-center rounded border border-ds-line/50 bg-ds-card px-1.5 py-0.5 text-xs font-bold uppercase tracking-wide text-[var(--text-secondary)]">
                  Source: Rack
                </span>
              ) : null}
            </div>
          ...
          {zone === 'custody' && r.custodyStatus === CUSTODY_ON_FLOOR && r.issuedOperator?.trim() ? (
            <p className="mt-1 text-xs text-[var(--brand-primary)] font-medium">
          ...
          <p className="text-xs text-[var(--text-secondary)] mt-1 leading-tight">
          ...
          {zone === 'prep' ? (
            <>
              <button
                type="button"
                disabled={saving}
                className="mt-1.5 w-full py-1.5 rounded bg-[var(--success)] hover:opacity-90 text-white text-xs font-semibold disabled:opacity-50"
                onClick={() =>
              ...
              {mode === 'dies' ? (
                <button
                  type="button"
                  disabled={saving}
                  className="mt-1.5 w-full py-1.5 rounded border border-[var(--success)]/80 bg-[var(--success-bg)] text-[var(--success)] text-xs font-semibold hover:opacity-90 disabled:opacity-50"
                  onClick={() =>
                    void runTransition(
          ...
          {zone === 'inventory' ? (
            <>
              <p className="text-xs font-medium text-[var(--text-secondary)] mt-1 tabular-nums">
                Reuse cycles: {r.reuseCount ?? 0}
              </p>
              <button
                type="button"
                disabled={saving}
                className="mt-1.5 w-full py-1.5 rounded border border-[var(--warning)]/80 bg-[var(--warning-bg)] text-[var(--warning)] text-xs font-semibold hover:opacity-90 disabled:opacity-50"
                onClick={() =>
                  void runTransition(
          ...
          {zone === 'custody' ? (
            <div className="mt-1.5 flex flex-col gap-2">
              {custodyInUse ? (
                <p className="text-xs font-bold uppercase tracking-wide text-[var(--brand-primary)]">In use on press</p>
              ) : null}
              {r.custodyStatus !== 'on_floor' ? (
                <>
                  ...
                  <button
                    type="button"
                    disabled={saving}
                    className="w-full py-1.5 rounded bg-[var(--brand-primary)] hover:opacity-90 text-white text-xs font-bold shadow-sm disabled:opacity-50"
              ...
              <button
                ...
                className={`w-full py-1.5 rounded text-foreground text-xs font-bold shadow-sm ${
                  r.jobCardHub?.key === 'printed'
                    ? 'bg-[var(--success)] hover:opacity-90 ring-2 ring-[var(--success)]/40 text-white'
              ...
              <button
                type="button"
                className="w-full rounded border border-[var(--error)]/50 bg-[var(--error-bg)] py-1.5 text-xs font-semibold text-[var(--error)] hover:opacity-90"
                onClick={() => {
                  setScrapId(r.id)
                  setScrapReason('')
                }}
    ...
    return (
      ...
      >
        ...
        <div className="mt-1 min-w-0 space-y-0.5">
          {r.linkedProductId ? (
            <Link
              ...
              className="block truncate font-sans text-sm font-bold text-[var(--brand-primary)] hover:text-[var(--brand-primary-hover)] hover:underline"
            >
              {prodTitle}
            </Link>
          ) : (
            <span className="block truncate font-sans text-sm font-bold text-[var(--brand-primary)]">
              {prodTitle}
            </span>
          )}
          <p className={`text-xs text-ds-ink-muted ${HUB_EMB_MONO}`}>
            <span className="text-ds-ink-faint font-sans">Code: </span>
            <button
              type="button"
              className="text-ds-ink hover:text-[var(--brand-primary)] hover:underline"
        ...
        <div className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-0.5">
          {r.operationalStatus === 'In-Use' && r.issuedMachineId ? (
            <Link
              ...
              className="text-xs font-bold uppercase tracking-wide text-[var(--success)] hover:opacity-90 hover:underline"
            >
              In production
            </Link>
          ) : (
        ...
        {r.strikeOverLimit ? (
          <p className="mt-1 w-fit animate-pulse rounded border border-[var(--error)]/50 bg-[var(--error-bg)] px-1.5 py-0.5 text-xs font-bold uppercase tracking-wider text-[var(--error)]">
        ...
        {r.issuedMachineLabel ? (
          <p className={`mt-0.5 text-xs text-[var(--text-secondary)] ${HUB_EMB_MONO}`}>Press: {r.issuedMachineLabel}</p>
        ) : null}
        {renderSpecs(r, zone)}
        {zone === 'triage' ? (
          <>
        ...
        {zone === 'custody' ? (
          <div className="mt-1.5 flex flex-col gap-2">
            ...
            <button
              type="button"
              disabled={saving}
              className={`w-full py-1.5 rounded text-white text-xs font-bold shadow-sm ${
                r.jobCardHub?.key === 'printed'
                  ? 'bg-[var(--success)] hover:opacity-90 ring-2 ring-[var(--success)]/40'
  ...
  return (
    <div className="min-h-screen bg-background text-ds-ink p-4 md:p-6">
              ...
              <div className="mt-3 flex flex-col sm:flex-row sm:flex-wrap sm:items-end gap-3 max-w-xl">
                ...
                {mode === 'dies' ? (
                  <Link
                    ...
                    className="text-xs font-bold uppercase tracking-wide text-ds-warning hover:text-ds-warning border border-ds-warning/30 rounded-lg px-3 py-2 shrink-0"
                  >
                    Staff settings
                  </Link>
                ) : null}
            ...
            <div className="flex flex-col lg:flex-row flex-wrap gap-3 lg:items-end lg:justify-between">
              <div className="flex flex-col lg:flex-row flex-wrap gap-3 lg:items-end flex-1 min-w-0">
                <label className="block flex-1 min-w-[200px]">
                  <span className="text-xs uppercase tracking-wide text-[var(--text-secondary)] font-semibold">
                    Search
                  </span>
                  <input
                    value={ledgerSearch}
              ...
              <div className="flex flex-wrap gap-2 items-center shrink-0">
                {mode === 'dies' ? (
                  <button
                    ...
                    className="px-3 py-2 rounded-md border border-[var(--success)]/80 bg-[var(--success-bg)] text-[var(--success)] text-xs font-bold hover:opacity-90 whitespace-nowrap"
                  >
                    + Add Die
                  </button>
                ) : null}
          ...
          <>
            {mode === 'dies' ? (
              <div className="rounded-lg border border-ds-line/50 bg-ds-main/90 px-3 py-2.5 mb-4 flex flex-col sm:flex-row sm:items-end gap-3 sm:justify-between">
                ...
                <p className="text-xs text-[var(--text-secondary)] sm:pb-2">
            ...
            <section className="rounded-xl border-2 border-ds-line/50 bg-ds-main p-3">
              ...
              <input
                ...
                placeholder="Search…"
                className="mb-3 w-full px-3 py-2 rounded-md bg-card border border-ds-line/50 text-foreground text-sm placeholder:text-[var(--text-secondary)]"
              />
              <ul className="space-y-2 flex-1 min-h-0 overflow-y-auto pr-1 max-h-[min(26rem,calc(100vh-14rem))] xl:max-h-none">
                {triageF.length === 0 ? (
                  <li className="text-[var(--text-secondary)] text-sm">No jobs awaiting triage.</li>
                ) : (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 lg:gap-6 xl:min-h-[min(70vh,calc(100vh-14rem))] xl:items-stretch">
              <section className="rounded-xl border-2 border-ds-line/50 bg-ds-main p-3 flex flex-col min-h-[260px] xl:min-h-0 xl:h-full">
                <div className="flex flex-col gap-2 mb-2 min-w-0">
                  ...
                  <p className="text-xs text-[var(--text-secondary)]">{prepSub}</p>
                  {mode === 'dies' ? (
                    <button
                      type="button"
                      onClick={() => {
                      ...
                      className="w-full px-3 py-2 rounded-md border border-[var(--brand-primary)]/80 bg-[var(--brand-bg-soft)] text-[var(--brand-primary)] text-xs font-bold hover:opacity-90"
                    >
                      + Manual Vendor PO
                    </button>
                  ) : (
                    <button
                      ...
                      className="w-full px-3 py-2 rounded-md border border-ds-warning/50 bg-ds-warning/10 text-ds-ink text-xs font-bold hover:bg-ds-warning/12"
                    >
                      + Manual Request
                    </button>
                  )}
                ...
                <ul className="space-y-2 flex-1 min-h-0 overflow-y-auto pr-1 text-sm max-h-[min(26rem,calc(100vh-14rem))] xl:max-h-none">
                  {prepF.length === 0 ? (
                    <li className="text-[var(--text-secondary)] text-sm">Empty.</li>
                  ) : (
                    <AnimatePresence initial={false}>
                ...
                <div className="flex flex-col gap-2 mb-2 min-w-0">
                  ...
                  {mode === 'dies' ? (
                    <button
                      ...
                      className="w-full px-3 py-2 rounded-md border border-[var(--success)]/80 bg-[var(--success-bg)] text-[var(--success)] text-xs font-bold hover:opacity-90"
                    >
                      + Add Die
                    </button>
                  ) : null}
              ...
              <section className="rounded-xl border-2 border-ds-line/50 bg-ds-main p-3 flex flex-col min-h-[260px] xl:min-h-0 xl:h-full">
                <div className="flex flex-col gap-1 mb-0.5 min-w-0">
                  ...
                  />
                  <p className="text-xs font-bold uppercase tracking-wider text-[var(--brand-primary)] tabular-nums">
                    {custodyInUseCount} active on press
                  </p>
                </div>
                <p className="text-xs text-[var(--text-secondary)] mb-2">Staging · tools marked ready</p>
                <input
                  value={custSearch}
                ...
                <ul className="space-y-2 flex-1 min-h-0 overflow-y-auto pr-1 text-sm max-h-[min(26rem,calc(100vh-14rem))] xl:max-h-none">
                  {custF.length === 0 ? (
                    <li className="text-[var(--text-secondary)] text-sm">Nothing in staging.</li>
                  ) : (
                    <AnimatePresence initial={false}>
            ...
            <p className="mt-4 text-center text-xs text-[var(--text-secondary)] font-[family-name:var(--font-designing-queue)] tracking-tight px-2">
      ...
      {returnModal ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 p-4">
          <div className="ci-hub-modal-panel max-w-md max-h-[90vh]">
            ...
            />
            <div>
              <span className="block text-sm text-[var(--text-secondary)] mb-1">Condition</span>
              <div className="flex rounded-lg border border-ds-line/50 overflow-hidden p-0.5 bg-background/40">
            ...
            {(() => {
              ...
              return changed ? (
                <div className="space-y-2 rounded-lg border border-ds-warning/45 bg-ds-warning/8 p-3">
                  <p className="text-xs font-semibold text-ds-warning">Size change on return</p>
                  <label className="block text-sm text-[var(--text-secondary)]">
                  ...
                  <label className="block text-sm text-[var(--text-secondary)]">
                    ...
                    <textarea
                      ...
                      rows={2}
                      className="mt-1 w-full px-3 py-2 rounded-md bg-background border border-ds-line/50 text-foreground text-sm"
                    />
                  </label>
                </div>
              ) : null
      ...
      {scrapId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 p-4">
          <div className="ci-hub-modal-panel max-w-md border-[var(--error)]/50">
            <h3 className="ci-hub-modal-title text-[var(--error)] border-[var(--error)]/40">Scrap / damage</h3>
            <p className="text-xs text-[var(--text-secondary)] leading-snug">Record why this tool is removed from active inventory.</p>
            <textarea
              value={scrapReason}
            ...
            <div className="flex justify-end gap-2">
              ...
              <button
                ...
                className="px-3 py-2 rounded bg-[var(--error)] hover:opacity-90 text-white font-semibold disabled:opacity-50"
      ...
      {issueDieId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 p-4">
          <div className="ci-hub-modal-panel max-w-md border-[var(--brand-primary)]/35">
