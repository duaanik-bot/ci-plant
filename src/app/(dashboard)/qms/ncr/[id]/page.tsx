...
export default function NcrDetailPage() {
  ...
  if (!ncr) return <div className="p-4 text-ds-ink-muted">Loading…</div>
    ...
    <div className="p-4 max-w-4xl mx-auto space-y-4">
      ...
      <div className="rounded-xl bg-ds-card border border-ds-line/50 p-4">
        ...
        {ncr.quantityAffected != null && (
          <p className="text-ds-ink-muted text-sm mt-1">Quantity affected: {ncr.quantityAffected}</p>
        )}
        {ncr.qcRecord && (
          <p className="text-ds-ink-muted text-sm mt-1">
            Linked QC: {ncr.qcRecord.checkType} — {ncr.qcRecord.result}
          </p>
        )}
      </div>

      <form onSubmit={handleSave} className="rounded-xl bg-ds-card border border-ds-line/50 p-4 space-y-4">
        <h2 className="text-sm font-semibold text-ds-ink">CAPA</h2>
        <div className="grid md:grid-cols-2 gap-4 text-sm">
          <div>
            <label className="block text-ds-ink-muted mb-1">Root cause</label>
            <textarea
          ...
          <div>
            <label className="block text-ds-ink-muted mb-1">Corrective action</label>
            <textarea
          ...
          <div>
            <label className="block text-ds-ink-muted mb-1">Preventive action</label>
            <textarea
          ...
          <div className="space-y-2">
            <div>
              <label className="block text-ds-ink-muted mb-1">Assignee</label>
              <select
            ...
            <div>
              <label className="block text-ds-ink-muted mb-1">Due date</label>
              <input
                type="date"
                value={form.dueDate}
            ...
            <div>
              <label className="block text-ds-ink-muted mb-1">Status</label>
              <select
                value={form.status}
