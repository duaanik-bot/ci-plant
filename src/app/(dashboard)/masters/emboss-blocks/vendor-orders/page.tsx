...
/**
 * Emboss Block Vendor Orders — rebuilt with ERP design system
 * ─────────────────────────────────────────────────────────────
...
async function fetchOrders(): Promise<Order[]> {
  const res = await fetch('/api/emboss-vendor-orders')
  if (!res.ok) throw new Error('Failed to load emboss vendor orders')
...
export default function EmbossVendorOrdersPage() {
  ...
  const { data: rows = [], isLoading } = useQuery<Order[]>({
    queryKey: ['emboss-vendor-orders'],
    queryFn:  fetchOrders,
  })

  const refresh = () => void qc.invalidateQueries({ queryKey: ['emboss-vendor-orders'] })
  ...
  const columns = [
    ...
    },
    {
      ...
      label:  'Block',
      render: (o: OrderWithOverdue) => (
        <span className="text-ds-ink-muted text-sm">{o.blockType ?? '—'}</span>
      ),
    },
    {
      key:    'vendorName',
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {/* ── Page header ───────────────────────────────────────────────── */}
      <PageHeader
        title="Emboss Block Vendor Orders"
        subtitle="Track emboss block manufacturing orders with vendors — status, expected delivery, and overdue tracking"
      />
      ...
      <DataTable
        ...
        data={withOverdue}
        loading={isLoading}
        emptyMessage="No emboss vendor orders found."
      />
