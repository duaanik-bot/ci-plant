    ...
    <div className="p-4 max-w-7xl mx-auto space-y-4">
      ...
      <div className="space-y-4">
        {Object.entries(data).map(([rack, blocks]) => (
          <div key={rack} className="rounded-ds-lg bg-ds-card shadow-ds-depth-sm p-3">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-2">
              {Array.from({ length: 6 }, (_, i) => {
                ...
                if (!block) return <div key={shelf} className="rounded bg-ds-elevated/50 p-2 text-xs text-ds-ink-faint">EMPTY<br />{shelf}</div>
                return (
                  <Link key={shelf} href={`/masters/emboss-blocks/${block.id}`} className="rounded bg-ds-elevated p-2 text-xs">
