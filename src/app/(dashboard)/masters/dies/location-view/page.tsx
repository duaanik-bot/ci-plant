  ...
  return (
    <div className="p-4 max-w-7xl mx-auto space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold text-[var(--brand-primary)]">Die Location View</h1>
        <Link href="/masters/dies" className="px-3 py-2 rounded-ds-md bg-ds-elevated text-ds-ink text-sm">Back</Link>
      </div>
      <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search die or carton" className="w-full md:w-96 px-3 py-2 rounded bg-ds-elevated text-foreground text-sm" />
      <div className="space-y-4">
        {Object.entries(data).map(([rack, dies]) => (
          <div key={rack} className="rounded-ds-lg bg-ds-card p-3 shadow-ds-depth-sm">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-2">
              {Array.from({ length: 8 }, (_, i) => {
                ...
                if (!die) return <div key={compartment} className="rounded bg-ds-elevated/50 p-2 text-xs text-ds-ink-faint">EMPTY<br />{compartment}</div>
                return (
                  <Link key={compartment} href={`/masters/dies/${die.id}`} className="rounded bg-ds-elevated p-2 text-xs">
