...
export default function NewJobCardPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 space-y-4">
      ...
      <section className="rounded-xl border border-ds-line/50 bg-ds-card p-4 space-y-3">
        ...
        <div className="grid md:grid-cols-2 gap-3">
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2 rounded-lg bg-background border border-ds-line/60 text-foreground"
              disabled={!selectedPo}
            >
        ...
        <div className="grid md:grid-cols-4 gap-3 text-sm">
          {[
            ...
            <div key={k}>
              ...
              <input value={v} readOnly className="w-full px-3 py-2 rounded-lg bg-ds-elevated border border-ds-line/60 text-foreground" />
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="rounded-xl border border-ds-line/50 bg-ds-card p-4 space-y-3">
        ...
        <div className="grid md:grid-cols-4 gap-3 text-sm">
          <div><label className="block text-ds-ink-muted mb-1">Material Code</label><input readOnly value={autoSummary.materialCode} className="w-full px-3 py-2 rounded-lg bg-ds-elevated border border-ds-line/60" /></div>
          <div><label className="block text-ds-ink-muted mb-1">Parent Board Size</label><input readOnly value={autoSummary.sheetSize} className="w-full px-3 py-2 rounded-lg bg-ds-elevated border border-ds-line/60" /></div>
          <div><label className="block text-ds-ink-muted mb-1">Required Sheets</label><input value={requiredSheets} onChange={(e) => setRequiredSheets(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-background border border-ds-line/60" /></div>
          <div><label className="block text-ds-ink-muted mb-1">Wastage Sheets</label><input value={wastageSheets} onChange={(e) => setWastageSheets(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-background border border-ds-line/60" /></div>
          <div><label className="block text-ds-ink-muted mb-1">Reserved Sheets</label><input readOnly value={autoSummary.reserved} className="w-full px-3 py-2 rounded-lg bg-ds-elevated border border-ds-line/60" /></div>
          <div><label className="block text-ds-ink-muted mb-1">Shortage Sheets</label><input readOnly value={autoSummary.shortage} className="w-full px-3 py-2 rounded-lg bg-ds-elevated border border-ds-line/60" /></div>
          <div><label className="block text-ds-ink-muted mb-1">PR Status</label><input readOnly value={autoSummary.prStatus} className="w-full px-3 py-2 rounded-lg bg-ds-elevated border border-ds-line/60" /></div>
        </div>
      </section>
      ...
      <section className="rounded-xl border border-ds-line/50 bg-ds-card p-4 space-y-3">
        ...
        <div className="grid md:grid-cols-4 gap-3 text-sm">
          <div><label className="block text-ds-ink-muted mb-1">Coating</label><input readOnly value={autoSummary.coating} className="w-full px-3 py-2 rounded-lg bg-ds-elevated border border-ds-line/60" /></div>
          <div><label className="block text-ds-ink-muted mb-1">Emboss/Leaf</label><input readOnly value={autoSummary.emboss} className="w-full px-3 py-2 rounded-lg bg-ds-elevated border border-ds-line/60" /></div>
          <div><label className="block text-ds-ink-muted mb-1">Printing machine</label><input value={machineId} onChange={(e) => setMachineId(e.target.value)} placeholder="Machine ID (optional)" className="w-full px-3 py-2 rounded-lg bg-background border border-ds-line/60" /></div>
          <div><label className="block text-ds-ink-muted mb-1">Operator</label><input value={assignedOperator} onChange={(e) => setAssignedOperator(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-background border border-ds-line/60" /></div>
          <div><label className="block text-ds-ink-muted mb-1">Shift</label><select value={shift} onChange={(e) => setShift(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-background border border-ds-line/60"><option>A</option><option>B</option><option>C</option></select></div>
          <div><label className="block text-ds-ink-muted mb-1">Priority</label><select value={priority} onChange={(e) => setPriority(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-background border border-ds-line/60"><option>Low</option><option>Normal</option><option>High</option></select></div>
          <div className="md:col-span-2"><label className="block text-ds-ink-muted mb-1">Remarks</label><input value={remarks} onChange={(e) => setRemarks(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-background border border-ds-line/60" /></div>
          <div className="md:col-span-4"><label className="block text-ds-ink-muted mb-1">Internal notes</label><textarea value={internalNotes} onChange={(e) => setInternalNotes(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-background border border-ds-line/60 min-h-[90px]" /></div>
        </div>
      </section>
      ...
      <div className="sticky bottom-2 z-20 rounded-xl border border-ds-line/60 bg-background p-3 flex justify-end gap-2">
        <button type="button" onClick={() => router.push('/production/job-cards')} className="px-4 py-2 rounded-lg border border-ds-line/60 text-sm">Cancel</button>
        <button type="button" disabled={saving} onClick={() => void createJobCard(true)} className="px-4 py-2 rounded-lg border border-ds-line/60 text-sm">Save Draft</button>
        <button type="button" disabled={saving} onClick={() => void createJobCard(false)} className="px-4 py-2 rounded-lg bg-ds-warning text-primary-foreground text-sm">{saving ? 'Saving…' : 'Create Job Card'}</button>
      </div>
    </div>
  )
}
