    ...
    <div className="p-4 max-w-2xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="bg-slate-900 rounded-lg border border-slate-700 p-4 space-y-3">
          <h3 className="text-slate-300 text-sm font-medium">Received Quantity</h3>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm text-slate-400 mb-1">PO reference</label>
              <input type="text" value={poReference} onChange={(e) => setPoReference(e.target.value)} className={cls} placeholder="e.g. PO-2026-0042" />
            </div>
            <div>
              <label className="block text-sm text-slate-400 mb-1">PO qty ({entryUnit})</label>
        ...
        {toleranceWarning && (
          <div className="bg-red-900/40 border border-red-700 rounded-lg p-3 text-sm space-y-2">
            ...
            <div className="flex items-center gap-2">
              <input type="checkbox" id="approvalOverride" checked={approvalOverride} onChange={(e) => setApprovalOverride(e.target.checked)} className="rounded border-slate-600" />
        ...
        {/* Pricing */}
        <div className="bg-slate-900 rounded-lg border border-slate-700 p-4 space-y-3">
          <h3 className="text-slate-300 text-sm font-medium">Pricing</h3>
          <div className="grid grid-cols-2 gap-3">
            {isBoardType ? (
              <div>
                <label className="block text-sm text-slate-400 mb-1">Price per kg (\u20B9)</label>
          ...
          {isBoardType && conversion && pricePerKg && (
            <p className="text-[10px] text-slate-500">WAC will be calculated per kg: (Current kg &times; Current WAC + {conversion.totalWeightKg.toFixed(2)} kg &times; \u20B9{pricePerKg}) / total kg</p>
          )}
        </div>

        {/* Batch & Pallet Tracking */}
        <div className="bg-slate-900 rounded-lg border border-slate-700 p-4 space-y-3">
          <h3 className="text-slate-300 text-sm font-medium">Batch & Pallet Tracking</h3>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm text-slate-400 mb-1">Lot / Pallet ID</label>
            ...
            <div>
              <label className="block text-sm text-slate-400 mb-1">Mill date</label>
              <input type="date" value={millDate} onChange={(e) => setMillDate(e.target.value)} className={cls} />
            </div>
            <div>
              <label className="block text-sm text-slate-400 mb-1">Number of pallets</label>
