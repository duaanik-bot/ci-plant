...
export async function POST(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const dedupeKey = buildIssueDedupeKey(user.id, 'emboss_block:receive', toolId)
    ...
    clearIssueDedupeKey(buildIssueDedupeKey(user.id, 'emboss_block', toolId))
    return new NextResponse(safeJsonStringify({ ok: true }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' },
    })
  } catch (e) {
    console.error('[inventory-hub/emboss-blocks receive]', e)
