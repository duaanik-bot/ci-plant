...
export async function POST(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const dedupeKey = buildIssueDedupeKey(user.id, 'shade_card:vendor_receive', toolId)
    ...
    const result = await receiveToolFromVendor('shade_card', toolId, {
    ...
    console.error('[inventory-hub/shade-cards receive-from-vendor]', e)
