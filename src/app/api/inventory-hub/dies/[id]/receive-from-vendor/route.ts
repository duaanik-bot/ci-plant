...
export async function POST(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const dedupeKey = buildIssueDedupeKey(user.id, 'die:vendor_receive', toolId)
    ...
    const result = await receiveToolFromVendor('die', toolId, {
    ...
    console.error('[inventory-hub/dies receive-from-vendor]', e)
