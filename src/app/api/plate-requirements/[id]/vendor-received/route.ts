...
/** POST — Plates received back from vendor → incoming triage for next routing (CTP / stock / re-vendor). */
export async function POST(
  ...
  if (reqRow.triageChannel !== 'outside_vendor') {
    return NextResponse.json(
      { error: 'Only outside-vendor queue items can be marked received' },
      { status: 409 },
    )
  }
    ...
    await db.$transaction(async (tx) => {
      ...
      await tx.auditLog.create({
        data: {
          ...
          recordId: id,
          newValue: { vendorReceivedToTriage: true } as object,
        },
      })

      await createPlateHubEvent(tx, {
        plateRequirementId: id,
        actionType: PLATE_HUB_ACTION.VENDOR_RECEIVED_TO_TRIAGE,
        fromZone: HUB_ZONE.OUTSIDE_VENDOR,
