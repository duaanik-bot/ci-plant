...
/** Shared hub row builder for JSON hub + CSV export (same search semantics). */
export async function queryShadeCardHubRows(searchQuery: string): Promise<ShadeCardSpotlightRow[]> {
  ...
  const rows = cards
    .map((c) => {
      ...
      return {
        ...
        issuedOperator: c.issuedOperator,
        industrialPriority,
        fadeAlert,
        deltaEAlert,
