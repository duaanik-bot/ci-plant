...
const input: PmInput[] = [
  { machine: 'PRN-01', scheduled: 4, completed: 4, overdue: 0 },
  { machine: 'PRN-02', scheduled: 4, completed: 2, overdue: 1 },
]

describe('computePmCompliance', () => {
  const r = computePmCompliance(input)
  it('computes compliance % per machine', () => {
    expect(r.rows.find((x) => x.machine === 'PRN-01')!.compliancePct).toBe(100)
    expect(r.rows.find((x) => x.machine === 'PRN-02')!.compliancePct).toBe(50)
  })
